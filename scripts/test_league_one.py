"""
Targeted backfill for League One (API-Sports ID 41) — 4 API calls only.
"""
import os
import requests
import psycopg2
...
def main():
    with psycopg2.connect(DB) as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT id FROM leagues WHERE name = 'League One'")
            row = cur.fetchone()
    if not row:
        print("League One not found in DB")
        return
    local_league_id = row[0]
    print(f"League One local ID: {local_league_id}")

    for season in SEASONS:
        print(f"Fetching League One season {season}...")
        r = requests.get(f"{BASE_URL}/fixtures?league=41&season={season}", headers=HEADERS)
        r.raise_for_status()
        fixtures = r.json().get("response", [])
        print(f"  {len(fixtures)} fixtures from API")

        conn = get_conn()
        season_fixtures = season_results = 0
        with conn.cursor() as cur:
            for f in fixtures:
